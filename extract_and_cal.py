...
REQUIREMENTS:
    ...
    (2) now quit CASA, then reopen it
        >>> import pip
        >>> pip.main(['install', 'astroquery==0.3.4', '-Iv', '--user'])
...
# results = alma.query(payload=dict(project_code='2019.2.00028.S'), public=False, cache=False)

# # mask = results['Source name'] == 'NGC_253'
...
for dirpath, dirnames, filenames in os.walk('2019.2.00028.S'):
    for fn in filenames:
